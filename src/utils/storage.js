...
export function getLast7DaysCardsReviewed(){var result=[];var cards=getCards();for(var i=6;i>=0;i--){var d=new Date();d.setDate(d.getDate()-i);var ds=d.toISOString().split('T')[0];var count=0;cards.forEach(function(c){if(c.lastReviewed&&c.lastReviewed.split('T')[0]===ds)count++});result.push({date:ds,label:d.toLocaleDateString('en-US',{weekday:'short'}),count:count})}return result}
